...
describe("FirestoreDLsiteWorkSchema", () => {
  ...
  it("必須フィールドが不足している場合エラーが発生する", () => {
    ...
    delete (invalidData as any).productId;
...
describe("FrontendDLsiteWorkSchema", () => {
  it("有効なフロントエンド作品データを検証できる", () => {
    ...
    if (result.success) {
      ...
      expect(result.data.relativeUrl).toBe(
        "/maniax/work/=/product_id/RJ236867.html",
      );
    }
  });
  ...
  it("フロントエンド固有フィールドが必須である", () => {
    ...
    const result = FrontendDLsiteWorkSchema.safeParse(
...
describe("convertToFrontendWork", () => {
  it("Firestoreデータを正しくフロントエンド形式に変換できる", () => {
    ...
    expect(result.downloadText).toBe("DL8,000");
  ...
  it("評価なしの作品も正しく処理できる", () => {
    ...
    delete (workWithoutRating as any).rating;
  ...
  it("ダウンロード数の優先順位を正しく処理できる", () => {
    ...
    delete (workWithSales as any).totalDownloadCount;
    ...
    delete (workWithoutCounts as any).totalDownloadCount;
    delete (workWithoutCounts as any).salesCount;
