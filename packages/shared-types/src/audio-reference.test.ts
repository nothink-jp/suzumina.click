...
describe("AudioReferenceCategorySchema", () => {
  it("有効なカテゴリを検証できる", () => {
    ...
    validCategories.forEach((category) => {
      expect(AudioReferenceCategorySchema.safeParse(category).success).toBe(
        true,
      );
    });
  });
  ...
  it("無効なカテゴリでエラーが発生する", () => {
    expect(AudioReferenceCategorySchema.safeParse("invalid").success).toBe(
      false,
    );
...
describe("FirestoreAudioReferenceSchema", () => {
  it("有効なFirestore音声リファレンスデータを検証できる", () => {
    const result = FirestoreAudioReferenceSchema.safeParse(
...
describe("FrontendAudioReferenceSchema", () => {
  it("有効なフロントエンド音声リファレンスデータを検証できる", () => {
    const result = FrontendAudioReferenceSchema.safeParse(
      validFrontendAudioReference,
    );
    expect(result.success).toBe(true);
    if (result.success) {
...
describe("extractYouTubeVideoId", () => {
  ...
  it("11文字以外の動画IDは無効", () => {
    const invalidId = extractYouTubeVideoId(
