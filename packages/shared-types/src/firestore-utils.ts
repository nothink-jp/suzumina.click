...
export function toServerModel(
  ...
  const toTimestamp = (dateString: unknown) => {
    if (!dateString) return firestore.Timestamp.now();
    try {
      return firestore.Timestamp.fromDate(new Date(String(dateString)));
    } catch (e) {
      console.error("日時の変換に失敗しました:", e);
  ...
  return {
    ...
    liveBroadcastContent: data.liveBroadcastContent as
      | LiveBroadcastContent
      | undefined,
  };
}
 ...
 * @returns クライアント用のデータ
 */
export function fromServerModel(
  ...
  const toISOString = (timestamp: unknown) => {
    ...
    try {
      ...
      console.error("タイムスタンプの変換に失敗しました:", e);
