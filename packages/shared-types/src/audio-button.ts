 ...
 * 音声ボタン更新時の入力データスキーマ
 */
export const UpdateAudioButtonInputSchema =
  CreateAudioButtonInputSchema.partial().extend({
    id: z.string().min(1),
  });
...
export const AudioButtonQuerySchema = z.object({
  ...
  sortBy: z
    .enum(["newest", "oldest", "popular", "mostPlayed"])
    .default("newest"),
...
export type FirestoreAudioButtonData = z.infer<
  typeof FirestoreAudioButtonSchema
>;
export type FrontendAudioButtonData = z.infer<typeof FrontendAudioButtonSchema>;
export type CreateAudioButtonInput = z.infer<
  typeof CreateAudioButtonInputSchema
>;
export type UpdateAudioButtonInput = z.infer<
  typeof UpdateAudioButtonInputSchema
>;
 ...
 */
export function serializeAudioButtonForRSC(
  data: FrontendAudioButtonData,
): string {
 ...
 * RCCでのデシリアライズ関数
 */
export function deserializeAudioButtonForRCC(
  serialized: string,
): FrontendAudioButtonData {
  try {
    ...
    console.error("音声ボタンデシリアライズまたはスキーマ検証エラー:", error);
 ...
 * リスト結果のシリアライズ関数
 */
export function serializeAudioButtonListResult(
  result: AudioButtonListResult,
): string {
 ...
 * リスト結果のデシリアライズ関数
 */
export function deserializeAudioButtonListResult(
  serialized: string,
  ...
  try {
    ...
    console.error("音声ボタンリストのデシリアライズエラー:", error);
