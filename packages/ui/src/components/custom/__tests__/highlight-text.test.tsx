...
describe("HighlightText", () => {
	...
	it("大文字小文字を区別せずにハイライトする", () => {
		...
		expect(highlighted).toHaveClass("bg-primary/20", "text-foreground");
	...
	it("複数の一致する部分をハイライトする", () => {
		...
		highlightedElements.forEach((element) => {
			expect(element).toHaveClass("bg-primary/20", "text-foreground");
		});
	});

	it("日本語テキストでハイライトする", () => {
		render(<HighlightText text="こんにちは世界" searchQuery="世界" />);
		expect(screen.getByText("世界")).toHaveClass("bg-primary/20", "text-foreground");
		expect(screen.getByText("こんにちは")).toBeInTheDocument();
	});

	it("部分一致でハイライトする", () => {
		render(<HighlightText text="Hello World" searchQuery="ell" />);
		expect(screen.getByText("ell")).toHaveClass("bg-primary/20", "text-foreground");
	...
	it("特殊文字を含む検索語でハイライトする", () => {
		...
		expect(screen.getByText("World")).toHaveClass("bg-primary/20", "text-foreground");
	});

	it("検索語がテキスト全体と一致する場合", () => {
		render(<HighlightText text="Hello" searchQuery="Hello" />);
		expect(screen.getByText("Hello")).toHaveClass("bg-primary/20", "text-foreground");
	});
