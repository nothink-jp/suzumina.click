...
describe('HeadlessUiDisclosureExample Component', () => {
  it('should render the button and initially hide the panel content', () => {
    ...
    // Assert
    // ボタンが表示されていることを確認
    expect(screen.getByRole('button', { name: /Headless UI Disclosure サンプル/i })).toBeInTheDocument();
    // パネルの内容が最初は表示されていないことを確認 (getBy はエラーになるので queryBy を使う)
    expect(screen.queryByText(/これは Headless UI の Disclosure コンポーネントのサンプルです。/i)).not.toBeInTheDocument();
  });
  ...
  it('should show the panel content when the button is clicked', async () => {
    ...
    render(<HeadlessUiDisclosureExample />);
    const button = screen.getByRole('button', { name: /Headless UI Disclosure サンプル/i });
    ...
    expect(await screen.findByText(/これは Headless UI の Disclosure コンポーネントのサンプルです。/i)).toBeInTheDocument();
  });
