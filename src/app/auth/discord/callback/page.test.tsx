...
vi.mock("./CallbackClient", () => ({
  default: () => <div data-testid="mock-callback-client">モックコールバッククライアント</div>,
}));
...
describe("DiscordCallbackPageコンポーネント", () => {
  test("Suspenseでラップされたコンポーネントが正しくレンダリングされること", () => {
    ...
    const loadingIndicator = screen.getByText("", { selector: "span.loading.loading-dots" });
    ...
    expect(callbackClient.textContent).toContain("モックコールバッククライアント");
  });
  ...
  test("LoadingFallbackコンポーネントが正しく表示されること", () => {
    ...
    // Assert
    // 見出しが正しく表示されていることを確認
    expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent("Discord 認証");
