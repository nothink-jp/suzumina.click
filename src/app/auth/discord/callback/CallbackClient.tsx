...
export default function CallbackClient() {
  ...
  useEffect(() => {
    ...
    const functionsUrl = process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_AUTH_CALLBACK_URL ||
    ...
    .then(async (res) => {
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ error: "不明なサーバーエラー" }));
    ...
    .then((data) => {
      if (!data.success || !data.customToken) {
        throw new Error(data.error || "カスタムトークンの取得に失敗しました。");
      }
