...
vi.mock("@/lib/firebase/client", () => ({
  get auth() { return mockAuth; } // ゲッターを使用してテスト中に値を変更できるようにする
}));
...
describe("CallbackClientコンポーネント", () => {
  ...
  beforeEach(() => {
    ...
    // 環境変数のモック
    process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_AUTH_CALLBACK_URL = "https://test-functions-url.com";
  });
  ...
  test("APIリクエストに成功した場合、認証とリダイレクトが行われること", async () => {
    ...
    // 非同期処理が完了するまで待機
    await waitFor(() => {
      expect(screen.getByText("認証に成功しました！ホームページにリダイレクトします...")).toBeInTheDocument();
    });
    ...
    expect(signInWithCustomToken).toHaveBeenCalledWith(expect.anything(), "test-custom-token");
  ...
  test("レスポンスが成功でもカスタムトークンがない場合はエラーを表示すること", async () => {
    ...
    // エラーメッセージが表示されることを確認
    expect(screen.getByText(/カスタムトークンの取得に失敗しました/)).toBeInTheDocument();
  });
  ...
  test("authがnullの場合はエラーメッセージを表示すること", async () => {
    ...
    // エラーメッセージが表示されることを確認
    expect(screen.getByText(/認証システムの初期化に失敗しました/)).toBeInTheDocument();
  });
  ...
  test("サインイン処理でエラーが発生した場合はエラーメッセージを表示すること", async () => {
    ...
    (signInWithCustomToken as vi.Mock).mockRejectedValue(new Error("認証エラー"));
    ...
    // コンソールエラーが呼び出されることを確認
    expect(consoleErrorMock).toHaveBeenCalledWith("Authentication failed:", expect.any(Error));
  });
});