...
// Discord ユーザー情報の型を定義 (email, avatar を追加)
interface DiscordUser {
  id: string;
  username: string;
  avatar: string | null; // アバターハッシュ (null の場合あり)
  email: string | null; // email スコープが必要 (null の場合あり)
  // 他に必要なプロパティがあれば追加
}

// Discord ギルド情報の型を定義 (必要最低限)
...
// Discord アバター URL を生成するヘルパー関数
function getDiscordAvatarUrl(userId: string, avatarHash: string | null): string | undefined {
    if (!avatarHash) {
        return undefined;
    }
    const format = avatarHash.startsWith("a_") ? "gif" : "png"; // アニメーション GIF 対応
    return `https://cdn.discordapp.com/avatars/${userId}/${avatarHash}.${format}?size=128`; // サイズは適宜調整
}


// Discord 認証コールバック関数
// エンドポイント: /discord/auth/callback
export const discordAuthCallback = onRequest(
  {
    region: "asia-northeast1", // デプロイリージョンを指定
    secrets: [
