...
interface AudioClipListClientProps {
  ...
  getAudioClipsAction: (params: {
    videoId: string;
    limit?: number;
    startAfter?: Date | null;
  }) => Promise<{
    clips: ServerActionClipData[];
    hasMore: boolean;
    lastClip?: ServerActionClipData;
  }>;
  ...
  // 初期データ（オプション）
  initialClips?: ServerActionClipData[];
  initialHasMore?: boolean;
  initialLastClip?: ServerActionClipData;
}

/**
 * 音声クリップ一覧コンポーネント（クライアント）
 *
...
export default function AudioClipListClient({
  ...
  const [clips, setClips] = useState<AudioClip[]>(
    initialClips.map(convertToAudioClip),
  );
  const [hasMore, setHasMore] = useState(initialHasMore);
  const [lastClip, setLastClip] = useState<AudioClip | undefined>(
    initialLastClip ? convertToAudioClip(initialLastClip) : undefined,
  );
  ...
  function convertToAudioClip(clip: ServerActionClipData): AudioClip {
    try {
  ...
  const loadClips = async (isLoadMore = false) => {
    try {
      ...
      // props経由で受け取ったServer Actionを使用してクリップ一覧を取得
