...
describe("YouTubePlayer", () => {
  ...
  describe("基本的なレンダリング", () => {
    it("videoIdが指定されていない場合はエラーメッセージが表示される", () => {
      ...
      expect(
        screen.getByText("動画IDが指定されていません"),
      ).toBeInTheDocument();
    });
    ...
    it("カスタムクラスが適用される", () => {
      render(
        <YouTubePlayer videoId="test-video-id" className="custom-class" />,
      );
    ...
    it("数値でのサイズ指定が正しく適用される", () => {
      render(
        <YouTubePlayer videoId="test-video-id" width={800} height={450} />,
      );
    ...
    it("文字列でのサイズ指定が正しく適用される", () => {
      render(
        <YouTubePlayer videoId="test-video-id" width="50vw" height="50vh" />,
      );
...
describe("useYouTubePlayer Hook", () => {
      ...
      function TestHandlerComponent() {
        ...
        return (
          <div>
            ...
            <span data-testid="state-handler">
              {typeof handlers.onStateChange}
            </span>
            <span data-testid="time-handler">
              {typeof handlers.onTimeUpdate}
            </span>
          </div>
        );
      }
  ...
  describe("制御関数", () => {
    it("全ての制御関数が存在する", () => {
      function TestControlsComponent() {
        ...
        return (
          <div>
            ...
            <span data-testid="volume-control">
              {typeof controls.setVolume}
            </span>
      ...
      expect(screen.getByTestId("volume-control")).toHaveTextContent(
        "function",
      );
      expect(screen.getByTestId("mute-control")).toHaveTextContent("function");
      expect(screen.getByTestId("unmute-control")).toHaveTextContent(
        "function",
      );
    });
  });
  ...
  describe("高度な機能テスト", () => {
    it("時間更新とインターバル管理", async () => {
      ...
      render(
        <YouTubePlayer videoId="interval-test" onTimeUpdate={onTimeUpdate} />,
      );
  ...
  describe("エラーハンドリングテスト", () => {
    it("無効なvideoIdでも適切にエラーメッセージを表示", () => {
      ...
      // プレイヤーコンテナは表示されない
      expect(
        document.querySelector(".youtube-player-container"),
