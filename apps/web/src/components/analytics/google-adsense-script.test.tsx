...
describe("GoogleAdSenseScript", () => {
	...
	it("開発環境でADSENSE_CLIENT_IDが未設定の場合に警告をログ出力する", async () => {
		...
		expect(warn).toHaveBeenCalledWith("Google AdSense Client ID not configured");
	});

	it("開発環境でADSENSE_CLIENT_IDが設定されている場合に情報をログ出力する", async () => {
		const { info } = await import("@/lib/logger");
		process.env.NODE_ENV = "development";
		process.env.NEXT_PUBLIC_ADSENSE_CLIENT_ID = "ca-pub-1234567890123456";

		render(<GoogleAdSenseScript />);

		expect(info).toHaveBeenCalledWith("Google AdSense Script loading with Client ID:", {
