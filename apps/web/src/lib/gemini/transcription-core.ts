 ...
 * 分割の強制（8秒以内）と間投詞の独立発話化は SPR-294 ベンチマークで確定した改善:
 * 旧プロンプトは短い叫び・相槌が最長51秒の発話に併合されて消えた（検索再現率 7%→15% に倍増）。
 * 発話数が増えるため呼び出し側の maxOutputTokens は 32768 が前提（16384 では密なチャンクが切断される）
 */
export function buildTranscriptionPrompt(): string {
	return `この動画クリップは日本語配信アーカイブの一部です。クリップ内の全ての発話を漏らさず文字起こししてください。
- 発話は短く区切ること: 1発話 = 一息・一文。長い発話も文の切れ目で必ず分割し、1発話を8秒以内に収める
- 「あはは」「うわー」「えっ？」「よし」のような相槌・笑い・叫び・独り言・間投詞も、省略せずそれぞれ独立した1発話として書き起こすこと（短い発声こそ重要）
- ゲーム音声・BGM・効果音が重なっていても、人の声が聞こえる箇所は全て拾うこと。無音・BGMのみの区間だけを除外する
