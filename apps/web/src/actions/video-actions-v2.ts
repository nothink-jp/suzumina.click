 ...
 *
 * VideoV2エンティティを使用したサーバーサイドの操作を提供します。
 ...
 * 単一動画取得のレスポンス型
 */
export interface GetVideoV2Response {
	success: boolean;
	video?: VideoV2;
	error?: string;
}
 ...
 * 複数動画取得のレスポンス型
 */
export interface GetVideosV2Response {
	success: boolean;
	videos?: VideoV2[];
	error?: string;
}
 ...
 */
export async function getVideoV2Action(videoId: string): Promise<GetVideoV2Response> {
	try {
		// 入力値検証
 ...
 */
export async function getVideosV2Action(videoIds: string[]): Promise<GetVideosV2Response> {
	try {
		// 入力値検証
		...
		// Video V2エンティティに変換
		// 注: 実際の変換は後のPRで実装
		const videosV2 = legacyVideos as unknown as VideoV2[];
