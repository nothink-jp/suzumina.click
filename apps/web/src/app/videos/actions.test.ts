...
describe("Videos Actions", () => {
  ...
  describe("getVideoTitles", () => {
    ...
    it("個別ドキュメントの処理エラーでも他のドキュメントは処理される", async () => {
      const { mockFirestore, mockCollection } = createMockFirestore();
      (getFirestore as Mock).mockReturnValue(mockFirestore);
      ...
      (convertToFrontendVideo as Mock)
        ...
        .mockReturnValueOnce(
          createMockFrontendVideo({ title: "Another Valid Video" }),
        );
    ...
    it("全体的なエラーが発生した場合は空の結果を返す", async () => {
    ...
    it("空のコレクションの場合は適切な結果を返す", async () => {
      ...
      expect(mockConsole.log).toHaveBeenCalledWith(
        "📹 [Videos] No videos found in Firestore",
      );
    });
  });
  ...
  describe("getVideoById", () => {
    ...
    it("動画が存在しない場合はnullを返す", async () => {
      ...
      expect(mockConsole.log).toHaveBeenCalledWith(
        "動画が見つかりません: videoId=non-existent-id",
      );
    });
