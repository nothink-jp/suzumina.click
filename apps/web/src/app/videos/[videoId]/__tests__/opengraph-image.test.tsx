...
describe("動画詳細の OG 画像（app/videos/[videoId]/opengraph-image.tsx）", () => {
	...
	it("動画取得成功時はタイトル・チャンネル名・再生時間・サムネイルを描画する", async () => {
		...
		expect(response.element.props.secondaryLine).toBe("涼花みなせ");
		expect(response.element.props.emphasisLine).toBe("1:23:45");
	...
	it("再生時間が無い場合は durationLabel が空文字になる", async () => {
		...
		expect(response.element.props.emphasisLine).toBe("");
