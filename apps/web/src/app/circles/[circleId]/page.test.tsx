...
describe("CirclePage", () => {
	...
	it("サークル情報と作品一覧を正しく表示する", async () => {
		vi.mocked(actions.getCircleWithWorksWithPagination).mockResolvedValue({
	...
	it("サークルが存在しない場合は404を表示する", async () => {
		...
		vi.mocked(actions.getCircleWithWorksWithPagination).mockResolvedValue(null);
	...
	it("メタデータが正しく生成される", async () => {
		vi.mocked(actions.getCircleInfo).mockResolvedValue(mockCircleData);
	...
	it("存在しないサークルのメタデータはデフォルト値を返す", async () => {
		vi.mocked(actions.getCircleInfo).mockResolvedValue(null);
