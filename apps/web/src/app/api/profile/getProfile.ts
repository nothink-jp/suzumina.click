...
export async function getProfile(uid?: string): Promise<UserProfile | null> {
  try {
    ...
    console.log(`Firestoreからユーザープロフィール情報を取得します: ${targetUid}`);
    
    // Firestoreからユーザープロフィール情報を取得
    const profileDoc = await firestore.collection("userProfiles").doc(targetUid).get();
    
    // デバッグ用：プロフィール情報の取得結果をログ出力
    console.log(`プロフィール情報の取得結果: ${profileDoc.exists ? '存在します' : '存在しません'}`);
    ...
    if (profileDoc.exists) {
      ...
      console.log(`ユーザー ${targetUid} のプロフィール情報がまだ作成されていません。デフォルト値を使用します。`);
    }

    // 他のユーザーのプロフィールを閲覧する場合、
    // 非公開設定のプロフィールは本人以外閲覧不可
    if (uid && currentUser?.uid !== uid && profileData && !profileData.isPublic) {
      return null;
    }
    ...
    const mergedProfile = {
      ...
      email: currentUser.email,
      preferredName: profileData?.siteDisplayName || currentUser.displayName || targetUid.substring(0, 8),
    };
