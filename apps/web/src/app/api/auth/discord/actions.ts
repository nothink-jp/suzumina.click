 ...
 * @returns 認証処理の結果
 */
export async function handleDiscordCallback(code: string): Promise<DiscordAuthResult> {
  try {
    // 環境変数の検証
    const { clientId, clientSecret, redirectUri, targetGuildId } = validateEnvironmentVariables();
    ...
    try {
      ...
      if (error.code === "auth/user-not-found") {
        ...
        // その他のエラーは再スロー
        console.error("Firebaseユーザーの更新/作成中にエラーが発生しました:", error);
        throw error;
      }
    }
