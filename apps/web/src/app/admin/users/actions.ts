 ...
 * ユーザーの有効/無効を切り替え
 */
export async function toggleUserActive(discordId: string): Promise<ActionResult<FrontendUserData>> {
	const admin = await requireAdmin();
...
export async function getUserStats(): Promise<{
	...
	try {
		...
		const [totalSnapshot, activeSnapshot, adminSnapshot, moderatorSnapshot] = await Promise.all([
			usersCollection.get(),
