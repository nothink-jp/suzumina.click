...
import type { FrontendUserData, UserListResult } from "@suzumina.click/shared-types";
...
function UserItem({ user, onUserUpdate }: UserItemProps) {
	...
	const [selectedRole, setSelectedRole] = useState<"member" | "moderator" | "admin">(user.role);
		...
		<tr className={`hover:bg-gray-50 ${user.isActive ? "" : "bg-red-50"}`}>
			...
			<td className="px-6 py-4 whitespace-nowrap">
				<div className="space-y-1">
					...
					<span
						className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
							user.isPublicProfile ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"
						}`}
					>
						{user.isPublicProfile ? "公開" : "プライベート"}
					</span>
				</div>
			</td>
...
export default function UserList({ data, currentPage }: UserListProps) {
	...
	const handleUserUpdate = (updatedUser: FrontendUserData) => {
		setUsers((prevUsers) =>
			prevUsers.map((user) => (user.discordId === updatedUser.discordId ? updatedUser : user)),
		);
	};
			...
			<div className="overflow-x-auto">
				<table className="min-w-full divide-y divide-gray-200">
					...
					<tbody className="bg-white divide-y divide-gray-200">
						{users.map((user) => (
							<UserItem key={user.discordId} user={user} onUserUpdate={handleUserUpdate} />
						))}
					</tbody>
				</table>
			</div>
