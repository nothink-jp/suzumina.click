...
export default function VideoDetail({ video }: VideoDetailProps) {
	...
	const getLiveBroadcastDisplayName = (liveBroadcastContent?: string) => {
		...
		return typeMap[liveBroadcastContent || ""] || liveBroadcastContent || "不明";
	};
		...
		<div className="max-w-6xl mx-auto">
						...
						<div className="lg:col-span-2">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
								<div>
									...
									<p className="text-lg text-gray-900 mb-3">{video.channelTitle}</p>

									<div className="text-sm text-gray-500">
										<p>
											チャンネルID: <span className="font-mono">{video.channelId}</span>
										</p>
									</div>
								</div>
								...
								<div>
									...
									<div className="text-sm text-gray-500">
										<p>公開日: {new Date(video.publishedAtISO).toLocaleDateString("ja-JP")}</p>
										<p>公開時刻: {new Date(video.publishedAtISO).toLocaleTimeString("ja-JP")}</p>
									</div>
								</div>
							</div>
							...
							{video.description && (
								<div className="mb-8">
									<h3 className="text-lg font-medium text-gray-900 mb-4">動画説明</h3>
				...
				<div className="bg-white rounded-lg shadow-sm border p-6">
					...
					<dl className="space-y-4">
						<div>
							...
							<dd className="mt-1 text-sm text-gray-900 font-mono">{video.videoId}</dd>
						</div>

						<div>
							<dt className="text-sm font-medium text-gray-500">Firestore ID</dt>
							<dd className="mt-1 text-sm text-gray-900 font-mono">{video.id}</dd>
						</div>

						<div>
							<dt className="text-sm font-medium text-gray-500">チャンネルID</dt>
							<dd className="mt-1 text-sm text-gray-900 font-mono">{video.channelId}</dd>
						</div>
			...
			{thumbnailQualities.length > 0 && (
				<div className="mt-8 bg-white rounded-lg shadow-sm border p-6">
					<h2 className="text-xl font-semibold text-gray-900 mb-6">サムネイル品質</h2>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{thumbnailQualities.map((quality, _index) => (
							...
							>
								<h3 className="text-sm font-medium text-gray-700 mb-3">{quality.name}</h3>
								<div className="mb-3">
									<ThumbnailImage
