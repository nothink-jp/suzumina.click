...
describe("CreatorPage", () => {
	...
	it("クリエイター情報と作品一覧を正しく表示する", async () => {
		vi.mocked(actions.getCreatorWithWorksWithPagination).mockResolvedValue({
	...
	it("クリエイターが存在しない場合は404を表示する", async () => {
		...
		vi.mocked(actions.getCreatorWithWorksWithPagination).mockResolvedValue(null);
	...
	it("メタデータが正しく生成される", async () => {
		vi.mocked(actions.getCreatorInfo).mockResolvedValue(mockCreatorInfo);
	...
	it("存在しないクリエイターのメタデータはデフォルト値を返す", async () => {
		vi.mocked(actions.getCreatorInfo).mockResolvedValue(null);
