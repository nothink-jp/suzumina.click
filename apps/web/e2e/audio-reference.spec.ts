...
test.describe("音声ボタン機能", () => {
	test("音声ボタン一覧ページが表示される", async ({ page }) => {
		...
		// 検索フォームが表示される
		const searchPanel = page.locator('[data-testid="search-panel"]').or(page.getByRole("search"));
		await expect(searchPanel).toBeVisible();

		// 音声ボタン作成リンクが表示される
		await expect(page.getByRole("link", { name: /音声ボタンを作成/ })).toBeVisible();
	});
	...
	test("動画IDなしでアクセスした場合のエラー表示", async ({ page }) => {
		...
		// 動画一覧への誘導リンクが表示される
		await expect(page.getByRole("link", { name: /動画一覧から選ぶ/ })).toBeVisible();
	});
	...
	test("タイムスタンプ選択機能", async ({ page }) => {
		...
		await expect(page.getByRole("button", { name: /現在時刻を終了時間に設定/ })).toBeVisible();
	});
	...
	test("タグ入力機能", async ({ page }) => {
		...
		// タグ削除ボタンが表示される
		await expect(page.getByRole("button", { name: /E2Eテストを削除/ })).toBeVisible();
	});
	...
	test("検索とフィルタリング機能", async ({ page }) => {
		...
		// カテゴリフィルター（存在する場合）
		const categoryFilter = page.getByLabel(/カテゴリ/).or(page.locator('select[name="category"]'));
