...
async function fetchDLsiteData(): Promise<{
	...
	try {
		...
		try {
			...
			console.warn("JSONレスポンスの解析に失敗、HTMLから直接データを抽出します");
...
describe("DLsite Parser", () => {
	...
	beforeAll(async () => {
		...
		try {
			...
			console.log(`DLsiteから最新データを取得しました: ${new Date().toISOString()}`);
		} catch (error) {
			console.error("DLsiteへのアクセスに失敗しました:", error);
			throw new Error(`テスト実行にはDLsiteからのライブデータが必要です: ${error}`);
		}
	}, 30000); // 30秒のタイムアウト
	...
	describe("parseWorksFromSearchResult", () => {
		...
		it("涼花みなせの作品を正しく識別できる", () => {
			...
			const suzukaWorks = works.filter(
				(work) => work.author === "涼花みなせ" || work.title.includes("涼花みなせ"),
			);
		...
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Test case requires comprehensive validation
		it("価格情報を正しく抽出できる", () => {
			const works = parseWorksFromSearchResult(sampleData);
	...
	describe("データ整合性チェック", () => {
		it("抽出された全作品のデータ整合性を確認", () => {
			...
			for (const [index, work] of works.entries()) {
				...
				// 数値フィールドの確認
				expect(work.currentPrice, `作品${index}: currentPrice`).toBeGreaterThanOrEqual(0);

				if (work.originalPrice !== undefined) {
					expect(work.originalPrice, `作品${index}: originalPrice`).toBeGreaterThan(0);
				}

				if (work.discount !== undefined) {
					expect(work.discount, `作品${index}: discount`).toBeGreaterThanOrEqual(0);
					expect(work.discount, `作品${index}: discount`).toBeLessThanOrEqual(100);
				}
				...
				// 配列フィールドの確認
				expect(Array.isArray(work.sampleImages), `作品${index}: sampleImages`).toBe(true);
			}
		});
	});
	...
	describe("parseSampleImages function", () => {
      ...
      `;
			...
			expect(works[0].sampleImages[0].thumb).toBe("//img.dlsite.jp/sample1.jpg");
			expect(works[0].sampleImages[0].width).toBe(600);
			expect(works[0].sampleImages[0].height).toBe(400);
			expect(works[0].sampleImages[1].thumb).toBe("//img.dlsite.jp/sample2.jpg");
	...
	describe("URL正規化テスト", () => {
      ...
      `;
			...
			const works = parseWorksFromHTML(mockHtml);
			expect(works).toHaveLength(1);
			expect(works[0].workUrl).toBe("https://www.dlsite.com/product_id/RJ123456.html");
	...
	describe("ヘルパー関数のユニットテスト", () => {
		...
		describe("extractNumberFromParentheses", () => {
			...
			it("複数の括弧がある場合は最初の数値を抽出する", () => {
				expect(extractNumberFromParentheses("評価 (123) その他 (456)")).toBe(123);
			});
		});
