...
describe("リトライユーティリティ", () => {
  ...
  describe("retryApiCall関数", () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(mockedWarn).toHaveBeenCalledWith(
          expect.stringContaining("API呼び出しに失敗しました"),
        );
      },
    );

    it(
      "カスタムリトライオプションが適用されること",
      { timeout: 2000 },
      async () => {
    ...
    it("カスタムのスキップコードが適用されること", async () => {
      ...
      // 404をスキップ対象に追加
      await expect(
        retryApiCall(apiCall, { skipRetryCodes: [404] }),
    ...
    it("最大リトライ回数に達した後は例外をスローすること", async () => {
      ...
      expect(mockedWarn).toHaveBeenCalledTimes(1);
      expect(mockedWarn).toHaveBeenCalledWith(
        expect.stringContaining("残りリトライ回数: 1"),
      );
    });
