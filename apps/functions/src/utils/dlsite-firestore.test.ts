...
describe("dlsite-firestore", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(dlsiteMapper.mapToFirestoreData).mockReturnValue(mockFirestoreWork);
	...
	describe("saveWorksToFirestore", () => {
		it("作品データの保存が呼び出される", async () => {
			// 既存データなしの場合をモック
			const mockCollection = vi.mocked(firestore.default.collection("dlsiteWorks"));
			...
			// 関数実行（エラーが発生しないことを確認）
			await expect(dlsiteFirestore.saveWorksToFirestore([mockWorkBase])).resolves.not.toThrow();
		});

		it("空の配列を渡した場合は正常に処理される", async () => {
			// 基本的なモック設定（空配列なので実際には使われない）
			const _mockCollection = vi.mocked(firestore.default.collection("dlsiteWorks"));
			const mockBatch = {
				set: vi.fn().mockReturnThis(),
				update: vi.fn().mockReturnThis(),
				commit: vi.fn().mockResolvedValue(undefined),
			...
			await expect(dlsiteFirestore.saveWorksToFirestore([])).resolves.not.toThrow();
		});
	});
	...
	describe("searchWorksFromFirestore", () => {
		...
		it("複数の検索条件を組み合わせて作品を取得できる", async () => {
			...
			expect(mockCollection.where).toHaveBeenCalledWith("circle", "==", "テストサークル");
			expect(mockQuery.where).toHaveBeenCalledWith("category", "==", "音声作品");
	...
	describe("saveWorksToFirestore - 詳細なバッチ処理", () => {
		it("大量データ（500件超）のバッチ処理を正しく分割実行する", async () => {
			...
			await expect(dlsiteFirestore.saveWorksToFirestore(largeWorksList)).resolves.not.toThrow();
		...
		it("バッチコミットでエラーが発生した場合にエラーを投げる", async () => {
			...
			await expect(dlsiteFirestore.saveWorksToFirestore([mockWorkBase])).rejects.toThrow(
				"作品データのFirestore保存に失敗",
			);
		});
	...
	describe("getExistingWorksMap - in句制限対応", () => {
		it("10件を超えるproductIdを正しく分割して取得する", async () => {
			...
			expect(mockWhere).toHaveBeenCalledWith("productId", "in", expect.any(Array));
		});
	});
	...
	describe("getWorksStatistics - 詳細なカテゴリ処理", () => {
		it("カテゴリが未定義の作品を'不明'として分類する", async () => {
			...
			vi.mocked(firestore.default.collection).mockReturnValue(mockCollection as any);
	...
	describe("エラーハンドリング", () => {
		it("getWorkFromFirestore でエラーが発生した場合", async () => {
			...
			await expect(dlsiteFirestore.getWorkFromFirestore("RJ123456")).rejects.toThrow(
				"作品データの取得に失敗: RJ123456",
			);
		});
