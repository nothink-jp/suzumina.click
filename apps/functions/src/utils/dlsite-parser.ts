...
export function extractCategoryFromClass(
  categoryElement: unknown,
): WorkCategory {
  const classNames =
    (categoryElement as { attr: (name: string) => string | undefined }).attr(
      "class",
    ) || "";
...
export function extractStarRating(ratingElement: unknown): number {
  const className =
    (ratingElement as { attr: (name: string) => string | undefined }).attr(
      "class",
    ) || "";
...
export function parseWorksFromHTML(html: string): ParsedWorkData[] {
  ...
  $("table.work_1col_table tr").each((index: number, element: unknown) => {
    try {
      ...
      // 独占配信フラグの判定
      const isExclusive = $item.find(".icon_lead_01.type_exclusive").length > 0;
