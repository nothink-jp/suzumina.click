...
describe("Video Mapper V2", () => {
	...
	describe("mapYouTubeToVideoEntity", () => {
		it("should map complete YouTube video data to Video Entity", () => {
			...
			expect(video?.statistics?.viewCount.toNumber()).toBe(1000000);
			expect(video?.tags.playlistTags).toEqual(playlistTags);
			expect(video?.tags.userTags).toEqual(userTags);
			expect(video?.tags.contentTags).toEqual(["tag1", "tag2"]);
		});
		...
		it("should map statistics correctly", () => {
			...
			expect(video?.statistics?.viewCount.toNumber()).toBe(1000000);
			expect(video?.statistics?.likeCount?.toNumber()).toBe(50000);
			expect(video?.statistics?.dislikeCount?.toNumber()).toBe(1000);
			expect(video?.statistics?.favoriteCount).toBe(100);
			expect(video?.statistics?.commentCount?.toNumber()).toBe(5000);
		});

		it("should map live streaming details correctly", () => {
			const video = mapYouTubeToVideoEntity(createYouTubeVideo());

			expect(video?.liveStreamingDetails?.scheduledStartTime).toEqual(
				new Date("2024-02-01T10:00:00Z"),
			);
			expect(video?.liveStreamingDetails?.actualStartTime).toEqual(
				new Date("2024-02-01T10:05:00Z"),
			);
			expect(video?.liveStreamingDetails?.actualEndTime).toEqual(new Date("2024-02-01T12:00:00Z"));
	...
	describe("mapYouTubeVideosToEntities", () => {
		it("should map multiple videos successfully", () => {
			...
			expect(videos[0].tags.playlistTags).toEqual(["playlist1"]);
			expect(videos[0].tags.userTags).toEqual(["user1"]);
			expect(videos[1].id).toBe("video2");
			expect(videos[1].tags.playlistTags).toEqual(["playlist2"]);
			expect(videos[1].tags.userTags).toEqual([]);
			expect(videos[2].id).toBe("video3");
			expect(videos[2].tags.playlistTags).toEqual([]);
			expect(videos[2].tags.userTags).toEqual(["user3"]);
		});
	...
	describe("Legacy format conversion", () => {
		...
		it("should convert legacy format to Video Entity", () => {
			...
			expect(video.statistics?.viewCount.toNumber()).toBe(500000);
		});
	});
