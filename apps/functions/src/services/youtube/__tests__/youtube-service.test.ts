...
import { createVideoService } from "../youtube-service";
...
describe("YouTube Service V2", () => {
	...
	describe("createVideoService", () => {
		it("should create service with API key from environment", () => {
			expect(() => createVideoService()).not.toThrow();
		});

		it("should create service with provided API key", () => {
			expect(() => createVideoService("custom-key")).not.toThrow();
		});

		it("should throw error if no API key is available", () => {
			delete process.env.YOUTUBE_API_KEY;
			expect(() => createVideoService()).toThrow("YouTube API key is required");
		});
	});
