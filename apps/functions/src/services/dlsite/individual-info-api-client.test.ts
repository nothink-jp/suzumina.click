...
describe("Individual Info API Client", () => {
	...
	describe("fetchIndividualWorkInfo", () => {
		...
		it("詳細ログオプションが有効な場合適切にログ出力される", async () => {
			const mockResponse = {
				ok: true,
				status: 200,
				text: vi.fn().mockResolvedValue(JSON.stringify([mockWorkData])),
			...
			const options: IndividualInfoAPIOptions = {
				enableDetailedLogging: true,
			};

			await fetchIndividualWorkInfo("RJ123456", options);

			// ログ関数が呼ばれることを確認
			const logger = await import("../../shared/logger");
			expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("API取得開始: RJ123456"));
			expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("API取得成功: RJ123456"));
		});
