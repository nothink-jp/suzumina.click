...
const mockYoutubeSearchList = vi
	.fn()
	.mockImplementation((_params: SearchListParams): Promise<SearchListResponse> => {
...
const mockYoutubeVideosList = vi
	.fn()
	.mockImplementation((_params: VideoListParams): Promise<VideoListResponse> => {
		// 非同期処理を簡略化して、タイムアウトを防ぐ
		return Promise.resolve({ data: { items: [] } });
	});
...
describe("fetchYouTubeVideos", () => {
	...
	it("Base64エンコードされたイベントデータを処理できること", async () => {
		...
		const newData = mockEvent.data ? { ...mockEvent.data, data: base64Data } : { data: base64Data };
	...
	it("複数ページの動画がある場合、最大ページ数まで取得すること", async () => {
		...
		expect(mockedLoggerInfo).toHaveBeenCalledWith(expect.stringContaining("最大ページ数"));
	...
	it("動画情報の一部が不足している場合はスキップすること", async () => {
		...
		// 警告ログが出力されること（2回）
		expect(mockedLoggerWarn).toHaveBeenCalledWith(
			expect.stringContaining("IDまたはスニペットが不足しているため動画をスキップします"),
			expect.anything(),
		);
