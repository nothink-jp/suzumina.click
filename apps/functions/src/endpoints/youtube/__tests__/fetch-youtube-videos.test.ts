/**
 * YouTube 動画取得エンドポイントのテスト（SPR-230: uploads playlist incremental discovery・週次フルスイープ 中心）
 *
...
const { fetchYouTubeVideos } = await import("../fetch-youtube-videos");
...
describe("fetchYouTubeVideos: incremental discovery（uploads playlist）", () => {
...
describe("fetchYouTubeVideos: 配信中/配信予定の高速反映（mode=fast_recheck）", () => {
	it("stale live/upcoming動画のみを再取得・保存し、統計ティア更新は行わない（uploads playlist ID未キャッシュなら新着発見もスキップ）", async () => {
		vi.mocked(youtubeFirestore.getStaleLiveVideoIds).mockResolvedValue({
			videoIds: ["live1"],
			truncated: false,
		});
	...
	it("通常runの二重実行ロック（isInProgress）を参照・更新しない（メタデータは読み取りのみ）", async () => {
		getMetadataMock.mockResolvedValue({
			exists: true,
			data: () => ({
				isInProgress: true,
	...
	describe("軽量新着発見（uploads playlist incremental）", () => {
