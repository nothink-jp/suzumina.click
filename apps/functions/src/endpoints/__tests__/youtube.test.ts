...
describe("fetchYouTubeVideos: 配信中/配信予定の再チェック（mode=stale_live_recheck）", () => {
	it("stale live/upcoming動画のみを再取得・保存し、新着発見や統計ティア更新は行わない", async () => {
		vi.mocked(youtubeFirestore.getStaleLiveVideoIds).mockResolvedValue({
			videoIds: ["live1"],
			truncated: false,
		});
		...
		await fetchYouTubeVideos(pubsubEvent({ mode: "stale_live_recheck" }));
	...
	it("通常runの二重実行ロック（isInProgress）を参照・更新しない", async () => {
		getMetadataMock.mockResolvedValue({
			exists: true,
			data: () => ({
				isInProgress: true,
		...
		// 通常runなら isInProgress=true でスキップされるが、stale_live_recheckは
		// メタデータを見ないため実行される（= ロック起因のupdateも発生しない）
		expect(youtubeFirestore.saveVideosToFirestore).toHaveBeenCalled();
