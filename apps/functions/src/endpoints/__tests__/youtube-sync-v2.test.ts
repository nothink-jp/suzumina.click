...
describe("YouTube Sync V2 Endpoint", () => {
	...
	it("should successfully sync videos on first run", async () => {
		...
		// Mock video data
		const mockVideo1 = createMockVideoV2("video1");
		const mockVideo2 = createMockVideoV2("video2");
	...
	it("should continue from next page token", async () => {
		...
		const mockVideo = createMockVideoV2("video1");
		mockYouTubeService.fetchChannelVideos.mockResolvedValue({
			videos: [mockVideo],
...
/**
 * Helper function to create mock VideoV2
 */
function createMockVideoV2(id: string): VideoV2 {
	return {
		id,
		metadata: {
