...
- **Server Actions優先**:
  - API Routesは可能な限り使用せず、Server Actionsを使用してサーバーサイドロジックを実装します。
  - フォーム処理、データ更新、認証チェックなどはServer Actionsによる実装を基本とします。
...
- **サーバーサイドロジック**: Next.js Server Actions (`"use server"`)
...
- **命名規則**:
  ...
  - **Server Actions**: `actions.ts` または `[機能名].actions.ts` (関連するコンポーネントと同じディレクトリに配置)
...
- **Server Actionsの実装規約**:
  - Server Actionはファイル単位またはインライン関数単位で`"use server"`ディレクティブを使用します。
  - 認証チェック、バリデーション、エラーハンドリングは必ず各Server Action内で実装します。
  - 戻り値は`{success: boolean, data?: T, error?: string}`の形式を基本とします。
  - データアクセスのトランザクション処理は必要に応じて適切に実装します。
...
- **Server Actions優先**: API Routesではなく、Server Actionsを使用してサーバーサイドロジックを実装します。
- **アクション実装場所**:
  - 単一のコンポーネントでのみ使用するアクションは、そのコンポーネントと同じディレクトリに配置します。
  - 複数のコンポーネントで共有するアクションは、`src/actions/[機能名]/`ディレクトリに配置します。
- **認証とバリデーション**:
  - すべてのServer Action内で認証チェックとデータバリデーションを行います。
  - Zod利用によるバリデーション実装を推奨します。
- **エラーハンドリング**:
  - エラーは適切にキャッチし、ユーザーフレンドリーなメッセージとともに返します。
  - クライアントコンポーネントで適切にエラーを表示できるよう、一貫したエラー形式を使用します。
- **既存API Routesからの移行**:
  - 既存のAPI Routesは段階的にServer Actionsに移行します。
  - 移行後は不要になったAPI Routesを削除します。
