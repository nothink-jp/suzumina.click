...
```text
apps/web/src/app/
├── buttons/actions-v2.ts    # AudioButton V2用Server Actions
├── videos/actions-v2.ts     # Video V2用Server Actions
└── actions.ts               # フィーチャーフラグによる切り替え実装

apps/web/src/components/
├── audio/                   # V1/V2切り替えロジック追加
└── video/                   # V1/V2切り替えロジック追加
```

**内容**:
- フィーチャーフラグによるV1/V2切り替え実装
- actions-v2.tsファイルの作成（現在未実装）
- 環境変数でのフィーチャーフラグ制御
- 段階的ロールアウト対応
...
```text
apps/functions/src/services/youtube/
├── youtube-firestore-v2.ts  # Entity V2での保存処理
└── youtube-sync-v2.ts       # 更新（既存ファイル）

apps/functions/src/endpoints/
└── youtube.ts               # フィーチャーフラグによる切り替え
```

**内容**:
- 新規動画データをEntity V2形式で保存
- _v2Migrationフラグの自動付与
- フィーチャーフラグによるV1/V2切り替え
- 既存データとの整合性保持
