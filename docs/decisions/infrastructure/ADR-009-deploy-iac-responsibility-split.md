...
- **Cloud Run → Terraform が spec の正本**
  ...
  → terraform（`cloud_run.tf`）が spec と invoker の正本。GitHub Actions は image build/push と env/secret 投入のみ（`gcloud run deploy` から spec フラグを渡さない＝update セマンティクスで未指定フラグは既存 spec を保持する想定。**この patch 挙動は Cloud Run では本プロジェクト未実証のため、SPR-97 実装時に `gcloud run services describe` で要検証**）。
...
| SPR-95 | runtime の dead locals 削除（Actions 正本）※実装は SPR-92 に統合 | 1 |
