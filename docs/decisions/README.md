...
- [ADR-011: shadcn/ui の保守方針（再生成優先・未使用削除）](frontend/ADR-011-shadcn-ui-maintenance-policy.md) - SPR-61、生成物は手編集せず `add --overwrite`+Biome で再生成・未使用は削除（45→28）・class並べ替えchurnは追わない。ブランド色はコンポーネント外（トークン＋globals.cssの!important override）で適用＝在file例外は button(設計)/calendar(依存非互換) の2つのみ
