...
| Icon Stack | ReUI（無料） | `packages/ui/src/components/ui/icon-stack.tsx` | — | `pnpm dlx shadcn@latest add @reui/icon-stack --overwrite` | **導入済み**（2026-07-23） |
| Empty State | 手書き（Icon Stack を利用する合成コンポーネント） | `packages/ui/src/components/custom/empty-state.tsx` | 散在するインライン空表示 + 未使用だった `ListPageEmptyState` | — | **導入済み**（2026-07-23） |
...
- **API**: `icon?`/`title`/`description?`/`action?`/`className?`（旧 `ListPageEmptyState` と同一）に加え、
  2 プロパティを追加。`illustrated?: boolean`（既定 false）で icon を IconStack の装飾イラストに乗せるか
  切り替える（一覧ページの検索結果ゼロ・「まだ何もない」系は true、詳細ページ内セクションのような
  補助的な空表示は false）。`size?: "sm" | "default"` でコンパクト表示に対応
- **調査結果（UX監査）に基づく分類と対応**:
