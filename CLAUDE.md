...
- **完了前チェック**: 必ず **`pnpm verify`**（lint + typecheck + test を一括・各 `vitest.config.ts` のカバレッジ閾値も強制）。
  ...
  個別確認は `pnpm lint` / `pnpm typecheck` / `pnpm test`
...
- **実装の既定**: データ操作は Server Actions、表示は Server Components を優先、再利用コンポーネントは `packages/ui`
  ...
  - **書き込み系の認可ゲートの正本は `getCurrentUser()` の null チェック**で error を返す（SPR-195。
    `withAuthenticatedAction` ラッパーも同方式）。`requireAuth()` は `redirect()` を投げるため
    **Server Action の try/catch 内で使わない**（NEXT_REDIRECT が catch に飲まれる）。redirect は RSC/ページ側で
